  ...
  const merreShpalljetRuajtura = async () => {
    try {
      ...
      const response = await axios.get(
        `http://localhost:3000/api/punetRuajtura/shpalljet-e-ruajtura/${perdoruesiData._id}`,
      );
    ...
    <div className="container mx-auto px-4 py-8 max-w-7xl">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <Heart size={32} />
          <h1 className="text-4xl font-bold text-gray-800">Punët e Ruajtura</h1>
        </div>
      ...
      {shpalljetRuajtura.length === 0 ? (
        <div className="text-center py-16 bg-gray-50 rounded-xl">
          ...
          <p className="text-gray-500 text-lg">
            ...
            <span className="inline-block mx-1">📑</span> për të ruajtur punë që
            ju interesojnë
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {shpalljetRuajtura.map((ruajtur) => (
              <ShpalljaCard key={ruajtur._id} shpallja={ruajtur.shpalljaId} />
            ))}
          </div>
        </>
      )}
